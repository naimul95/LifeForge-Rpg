...
const COIN_REWARDS = { easy: 10, medium: 10, hard: 25, epic: 50 } as const;
const ACHIEVEMENTS = [{ name: "First 100 XP", description: "Earn 100 XP.", threshold: 100 }, { name: "Level 3", description: "Reach level 3.", threshold: 400 }, { name: "Level 5", description: "Reach level 5.", threshold: 1600 }];
...
export async function getRpgDashboardData(): Promise<RpgDashboardData> {
  ...
  const [profile, quests, achievements, dailyChallenge, coinTransactions] = await Promise.all([
    ...
    prisma.lifeCoinTransaction.findMany({ where: { userId }, orderBy: { createdAt: "desc" }, take: 10, select: { id: true, amount: true, reason: true, createdAt: true } }),
  ]);
  return { profile: { xp: profile.xp, coins: profile.coins, level: profile.level, nextLevelXp: levelForXp(profile.xp) ** 2 * 100 }, quests: quests.map((quest) => ({ id: quest.id, title: quest.title, description: quest.description, difficulty: quest.difficulty, period: quest.period, completed: quest.completed, xpReward: quest.xpReward, coinReward: quest.coinReward })), achievements: achievements.map((achievement) => ({ id: achievement.id, name: achievement.name, description: achievement.description, unlocked: achievement.userAwards.length > 0 })), dailyChallenge: { id: dailyChallenge.id, title: dailyChallenge.title, description: dailyChallenge.description, completed: Boolean(dailyChallenge.completedAt), xpReward: dailyChallenge.xpReward }, coinTransactions: coinTransactions.map((transaction) => ({ id: transaction.id, amount: transaction.amount, reason: transaction.reason, createdAt: transaction.createdAt.toISOString() })) };
}

export async function createQuest(input: unknown) {
  const { userId } = await requireSession(); const data = questInput.parse(input); await prisma.quest.create({ data: { userId, title: data.title, description: data.description, period: data.period, difficulty: data.difficulty, xpReward: XP_REWARDS[data.difficulty], coinReward: COIN_REWARDS[data.difficulty] } });
}

async function applyReward(transaction: Parameters<Parameters<typeof prisma.$transaction>[0]>[0], userId: string, rewardType: string, sourceId: string, xp: number, coins: number) {
  await transaction.rewardClaim.create({ data: { userId, rewardType, sourceId, xp, coins } });
  const profile = await transaction.profile.upsert({ where: { userId }, update: { xp: { increment: xp }, coins: { increment: coins } }, create: { userId, xp, coins, level: 1 } });
  const nextXp = profile.xp; await transaction.profile.update({ where: { userId }, data: { level: levelForXp(nextXp) } });
  if (coins) await transaction.lifeCoinTransaction.create({ data: { userId, amount: coins, reason: `${rewardType} reward`, referenceId: sourceId } });
  await unlockAchievements(transaction, userId, nextXp);
}

export async function completeQuest(questId: string) {
  const { userId } = await requireSession();
  try { await prisma.$transaction(async (transaction) => { const quest = await transaction.quest.findFirst({ where: { id: questId, userId } }); if (!quest || quest.completed) return; await transaction.quest.updateMany({ where: { id: questId, userId, completed: false }, data: { completed: true } }); await transaction.questCompletion.create({ data: { userId, questId } }); await applyReward(transaction, userId, "quest", quest.id, quest.xpReward, quest.coinReward); }); } catch (error) { if (!(error && typeof error === "object" && "code" in error && error.code === "P2002")) throw error; }
}
