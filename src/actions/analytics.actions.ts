...
export async function getAnalyticsDashboardData(): Promise<AnalyticsDashboardData> {
  ...
  const [
    ...
    prisma.studyDailyStat.findMany({
      where: { userId, dateKey: { gte: monthStart } },
      select: { dateKey: true, totalSeconds: true },
    }),
    ...
    prisma.goal.findMany({
      ...
      take: 5,
      select: { title: true, completionDate: true, updatedAt: true },
    }),
    prisma.questCompletion.findMany({
      where: { userId },
      orderBy: { completedAt: "desc" },
      take: 5,
      select: { completedAt: true, quest: { select: { title: true } } },
    }),
  ...
  const studySecondsByDay = new Map(
    stats.map((item) => [dayKey(item.dateKey), item.totalSeconds]),
  );
  const xpByDay = new Map<string, number>();
  for (const claim of claims) {
    const key = dayKey(claim.createdAt);
    xpByDay.set(key, (xpByDay.get(key) ?? 0) + claim.xp);
  }
  const habitCompletionsByDay = new Map<string, number>();
  for (const habit of habits)
    for (const log of habit.logs) {
      const key = dayKey(log.dateKey);
      const completed =
        habit.polarity === "good"
          ? log.status === "DONE" || log.completed
          : log.status === "AVOIDED";
      if (completed)
        habitCompletionsByDay.set(
          key,
          (habitCompletionsByDay.get(key) ?? 0) + 1,
        );
    }
  const weekly = Array.from({ length: 7 }, (_, index) => {
    ...
    return {
      ...
      studyMinutes: (studySecondsByDay.get(key) ?? 0) / 60,
      xp: xpByDay.get(key) ?? 0,
      habitCompletions: habitCompletionsByDay.get(key) ?? 0,
    };
  });
  ...
  const currentStreak = Math.max(
    ...habitStreaks.map((item) => item.currentStreak),
    0,
  );
  const bestStreak = Math.max(
    ...habitStreaks.map((item) => item.bestStreak),
    0,
  );
  const totalStudy = stats.reduce((sum, item) => sum + item.totalSeconds, 0);
  const heatmap = Array.from({ length: 28 }, (_, index) => {
    const date = new Date(monthStart);
    date.setUTCDate(date.getUTCDate() + index);
    const key = dayKey(date);
    return { date: key, minutes: (studySecondsByDay.get(key) ?? 0) / 60 };
  });
  const unlockedNames = new Set(
    userAwards.map((item) => item.achievement.name),
  );
  return {
    ...
    subjects: subjects.map((subject) => ({
      ...
      progress: subject.topics.length
        ? subject.topics.reduce((sum, topic) => sum + topic.completion, 0) /
          subject.topics.length
        : 0,
      studyMinutes: subject.studyMinutes,
    })),
    topics: subjects
      .flatMap((subject) =>
    ...
    achievements: achievements.map((achievement) => ({
      ...
      unlocked: unlockedNames.has(achievement.name),
    })),
    records: [
      { label: "Current habit streak", value: `${currentStreak} days` },
      { label: "Best habit streak", value: `${bestStreak} days` },
      {
        label: "Longest study session",
        value: `${Math.round((sessions[0]?.durationSeconds ?? 0) / 60)} min`,
      },
      { label: "Study this week", value: `${Math.round(totalStudy / 60)} min` },
      {
        label: "Habit check-ins",
        value: `${allLogs.filter((log) => log.completed).length}`,
      },
    ],
    ...
    journey: [
      ...goals.map((goal) => ({
        ...
        date:
          goal.completionDate?.toISOString() ?? goal.updatedAt.toISOString(),
      })),
      ...
      ...userAwards.map((item) => ({
        ...
        date: item.unlockedAt.toISOString(),
      })),
    ]
