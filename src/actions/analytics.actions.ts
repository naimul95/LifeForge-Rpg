...
export async function getAnalyticsDashboardData(): Promise<AnalyticsDashboardData> {
  ...
  const weekly = Array.from({ length: 7 }, (_, index) => { const date = new Date(weekStart); date.setUTCDate(date.getUTCDate() + index); const key = dayKey(date); return { label: date.toLocaleDateString("en-US", { timeZone: "UTC", weekday: "short" }), studyMinutes: (stats.find((item) => dayKey(item.dateKey) === key)?.totalSeconds ?? 0) / 60, xp: claims.filter((item) => dayKey(item.createdAt) === key).reduce((sum, item) => sum + item.xp, 0), habitCompletions: habits.reduce((sum, habit) => sum + habit.logs.filter((log) => dayKey(log.dateKey) === key && (habit.polarity === "good" ? log.status === "DONE" || log.completed : log.status === "AVOIDED")).length, 0) }; });
  ...
  const heatmap = Array.from({ length: 28 }, (_, index) => { const date = new Date(monthStart); date.setUTCDate(date.getUTCDate() + index); const key = dayKey(date); return { date: key, minutes: (stats.find((item) => dayKey(item.dateKey) === key)?.totalSeconds ?? 0) / 60 }; });
