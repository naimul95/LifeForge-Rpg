...
const goalInput = z
  ...
  .refine(
    (value) => value.deadline >= value.startDate,
...
export async function getPlanningData(
    ...
    ? await prisma.habit.findMany({
        ...
        select: {
          ...
          logs: {
            ...
            take: 90,
            select: { dateKey: true, completed: true, status: true },
          },
        },
      })
    : [];
  return {
    goals: goals.map((goal) => ({
      ...
      startDate: dateKey(goal.startDate),
  ...
  await prisma.$transaction(async (transaction) => {
    const updated = await transaction.goal.updateMany({
      ...
      data: {
        ...
        progress: 100,
        remaining: 0,
        completionDate: dateValue(dateKey(new Date())),
        xpAwarded: true,
      },
    });
    if (!updated.count) return;
    const claim = await transaction.rewardClaim.create({
      data: { userId, rewardType: "goal", sourceId: goalId, xp: 250, coins: 0 },
    });
    await transaction.profile.upsert({
      where: { userId },
      update: { xp: { increment: claim.xp } },
      create: { userId, xp: claim.xp, level: 1 },
    });
  });
}
...
export async function deleteGoal(goalId: string) {
  ...
  if (!result.count) throw new Error("Goal not found.");
}
...
export async function createHabit(input: unknown) {
  ...
  await prisma.habit.create({
    data: { userId, name: data.name, polarity: data.polarity },
  });
}
...
export async function toggleHabitLog(
  ...
  const { userId } = await requireSession();
  const day = z.string().date().parse(date);
  const habit = await prisma.habit.findFirst({
    where: { id: habitId, userId },
  });
  if (!habit) throw new Error("Habit not found.");
  await prisma.habitLog.upsert({
    where: { habitId_dateKey: { habitId, dateKey: dateValue(day) } },
    update: { completed },
    create: { userId, habitId, dateKey: dateValue(day), completed },
  });
}
...
export async function createReminder(input: unknown) {
  ...
  const remindAt = parseReminderDateTime(data.remindAt);
...
export async function toggleReminder(reminderId: string, completed: boolean) {
  ...
  if (!result.count) throw new Error("Reminder not found.");
}
...
export async function getDueReminders() {
  ...
  return prisma.reminder.findMany({
    where: { userId, completed: false, remindAt: { lte: new Date() } },
    orderBy: { remindAt: "asc" },
    take: 5,
    select: { id: true, title: true, description: true, remindAt: true },
  });
}
